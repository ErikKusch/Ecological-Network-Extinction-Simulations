...
FUN_Topo <- function(plot_df,
  ...
  if (sum(plot_df, na.rm = TRUE) == 0) {
    ...
    adj_mat <- bothtris[
      rownames(bothtris) %in% plants_sp,
...
FUN_SimComp <- function(PlantAnim = NULL, # should be set either to a vector of plant species names or animal species names
  ...
  if (file.exists(file.path(Dir.Exports, paste0(
    RunName, "SimulationNets_",
    IS, "_", Rewiring,
    "_CutOffs_", paste(unlist(CutOffs), collapse = "-"),
    ".RData"
  )))) {
    print("Extinctions already simulated")
    load(file.path(Dir.Exports, paste0(
      RunName, "SimulationNets_",
      IS, "_", Rewiring,
      "_CutOffs_", paste(unlist(CutOffs), collapse = "-"),
      ".RData"
    ...
    Sim_ls <- pblapply(names(AnalysisData_ls),
      ...
      function(y) {
        ...
        if (length(primext_namesC) != 0) {
          CustOrder_ExtC <- SimulateExtinctions(
            Network = net,
            Method = "Ordered",
            Order = primext_order,
            IS = IS,
          ...
          ExtC_Rand <- RandomExtinctions(
            Network = net, nsim = 200,
        ...
        if (length(primext_namesC) != 0) {
          ...
          CustOrder_ExtSML <- SimulateExtinctions(
            Network = net,
            Method = "Ordered",
            Order = primext_order,
            IS = IS,
          ...
          primext_order <- match(primext_namesS, rownames(x$Adjacency))
          CustOrder_ExtSLM <- SimulateExtinctions(
            Network = net,
            Method = "Ordered",
            Order = primext_order,
            IS = IS,
        ...
        Fun.Save <- function(x = CustOrder_ExtC,
          ...
          if (sum(is.na(as.matrix(x$Network))) != 0) {
            ...
            Rand[unlist(lapply(Rand, sum)) == 0 |
              unlist(lapply(Rand, FUN = function(k) {
                nrow(as.matrix(k))
              })) == 1] <- as.matrix(0)
          }
    ...
    save(Sim_ls, file = file.path(Dir.Exports, paste0(
      RunName, "SimulationNets_",
      IS, "_", Rewiring,
      "_CutOffs_", paste(unlist(CutOffs), collapse = "-"),
      ".RData"
...
FUN_TopoComp <- function(Sim_ls = NULL, RunName = "ALL", IS, Rewiring, CutOffs, Pre = PreExt_df) {
  ...
  MeanPred <- aggregate(. ~ netID + Proxy + Simulation,
    data = Topo_df[Topo_df$Simulation == "Prediction", ],
    FUN = mean, na.rm = TRUE, na.action = NULL
  )
  MeanRand <- aggregate(. ~ netID + Proxy + Simulation,
    data = Topo_df[Topo_df$Simulation == "Random", ],
    FUN = mean, na.rm = TRUE, na.action = NULL
  )
  SDRand <- aggregate(. ~ netID + Proxy + Simulation,
    data = Topo_df[Topo_df$Simulation == "Random", ],
  ...
  Save_ls <- list(
    Topo_ls = PostExt_ls, Topo_df = Topo_df,
    Eff_df = Eff_df, EffSD_df = EffSD_df
  )

  ## Data Return
  save(Save_ls, file = file.path(Dir.Exports, paste0(
    RunName, "SimulationTopo_",
    IS, "_", Rewiring,
    "_CutOffs_", paste(unlist(CutOffs), collapse = "-"),
    ".RData"
